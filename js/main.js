...
function convert() {
    ...
    .then(text => {
        ...
        var newmap = convertp2(JSON.parse(decompressed));
        if (newmap != null) {
            // copy new map to clipboard
            navigator.clipboard.writeText(newmap)
            .then(() => {
                console.log('New map copied to clipboard!');
                document.getElementById("output").textContent = newmap;
    ...
    .catch(err => {
        ...
        alert('Error: Failed to read level data (Error Code: Amogus)');
    });
}

function convertp2(map) {
    console.log(map);
    console.log("Loading template mapvar...");
    var templateMapvar = `{
        "textures": {
        ...
        "player_img": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAHJJREFUOE9jZMAC5jkx/McmnrSPgRFdHEUAWWOiK6rS+bsRfGSD4AbANKNrRLcRZhDMELABxGqGGYZsCNwAQjZjcwnIFYyk2o7uCrABpNqObMhwMQDkJ1LDARaVlEcjVRISKYZgTcqweKUoMyEnVVKyMwCd9EVLb4USZgAAAABJRU5ErkJgggAA",
        "tile_size": 16,
        "display_order": [
          2,
          6,
    ...
    console.log(map);
    var mapvar = templateMapvar.replace('["MAP DATA HERE"]', JSON.stringify(map));
    return mapvar;
}
...
function download() {
    ...
    // now we need to create a link to download the file
